...
app.get("/user/following", authenticationToken, async (request, response) => {
  ...
  const userFollowsQuery = `
            ...
            WHERE
                follower.follower_user_id = ${user_id}
            ;`;
  const userFollowsArray = await db.all(userFollowsArray);
...
app.get("/tweets/:tweetId", authenticateToken, async, (request, response) => {
  ...
  const userFollowersQuery = `
        SELECT
           *
        FROM  follower INNER JOIN user ON user.user_id = follower.following-user_id
  ...
  async (request, response) => {
    ...
    const getLikedUsersQuery = `
            SELECT
                *
            FROM
                follower INNER JOIN tweet ON tweet.user_id = follower.following_user_id INNER JOIN like ON like.tweet = tweet.tweet_id
  ...
  async (request, response) => {
    ...
    const getRepliedUsersQuery = `
            SELECT
               *
            FROM
                follower INNER JOIN tweet.user_id = follower.following_user_id INNER JOIN reply ON reply.tweet_id = tweet.tweet_id
...
app.delete("/tweets/:tweetId", authenticateToken, async (request, response) => {
  ...
  const { payload } = request;
  const { user_id, name, username, gender } = payload;
  const selectUserQuery = `SELECT * FROM tweet.user_id = ${user_id} AND tweet.tweet_id = ${user_id} AND tweet_id = ${tweetId};`;
